...
class TournamentPricingTest extends TestCase
{
    ...
    public function test_organizer_can_mark_tournament_as_paid_with_valid_fee(): void
    {
        ...
        $response = $this->actingAs($organizer)->put(
            route('tournaments.pricing.update', $tournament),
            ['is_paid' => 1, 'entry_fee' => 500]
        );

        $response->assertRedirect();
        $this->assertDatabaseHas('tournaments', ['id' => $tournament->id, 'is_paid' => 1, 'entry_fee' => 500]);
    }
    ...
    public function test_marking_free_forces_entry_fee_to_zero(): void
    {
        $organizer  = User::factory()->create(['role' => 'organizer']);
        $tournament = $this->makeTournament($organizer);
        $tournament->update(['is_paid' => true, 'entry_fee' => 300]);

        $this->actingAs($organizer)->put(route('tournaments.pricing.update', $tournament), ['is_paid' => 0]);

        $this->assertDatabaseHas('tournaments', ['id' => $tournament->id, 'is_paid' => 0, 'entry_fee' => 0]);
    }
    ...
    public function test_non_organizer_cannot_change_pricing(): void
    {
        ...
        $response = $this->actingAs($stranger)->put(
            route('tournaments.pricing.update', $tournament),
            ['is_paid' => 1, 'entry_fee' => 200]
        );
